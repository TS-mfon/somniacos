...
export default async function RevenuePage() {
  ...
  return (
    <>
      <PageHero title="Protocol Revenue" eyebrow="0.1 STT per transaction">Transparent fee accounting for SomniacOS agent usage.</PageHero>
      ...
      <section className="mt-5 panel rounded-[1.5rem] p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3">
          ...
          {!revenue.events.length ? <p className="rounded-2xl border border-dashed border-white/12 p-5 text-sm text-white/45">No indexed fee events yet. Run an agent from the Workbench to create a real protocol fee transaction.</p> : null}
        </div>
      </section>
    </>
  );
